...
export const DocumentForm: FC<DocumentFormProps> = ({
   ...
   const document: Partial<ProjectDocument> = project?.documents && index !== undefined ? project.documents[index] : {};
   ...
   const auth = useAppSelector(({auth})=> auth);
    ...
    const validationSchema = yup.object().shape({
        ...
        url: yup.string().required('This field is required').url("Must be a valid url"),
    });
    const initialValues: Omit<ProjectDocument, 'dateAdded' | 'addedById' | 'addedBy'> = {
               ...
               const newDoc: ProjectDocument = {
                ...
                if(mode === "add"){
                   ...
                   try {
                      ...
                      let err: any = error;
                      toast({title: "Could not add Document", description: err?.message || "Unknown Error", status: "error"})
                   }
                   finally{
                      onClose();
                   }
                ...
                if(mode === "edit"){
                   if(index === undefined) return
                    try {
                        ...
                        toast({title: "Successfully edited document", status: "success"});
                        setProject({...project, documents: newdocuments});
                    } catch (error) {
                        let err: any = error;
                        toast({title: "Could not edit proposal", description: err?.message || "Unknown Error", status: "error"})
                     }
                     finally{
                        onClose();
                     }
