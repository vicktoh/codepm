...
export const ImageCropper: FC<ImageCropperProps> = ({setInput, onClose, src}) => {
    const [crop, setCrop] = useState<Crop &{aspect?: number}>({unit: '%', width: 30, x: 0, y: 0, height: 30, aspect: 1/1});
    ...
    return (
        <Container centerContent maxWidth="md">
            ...
            <Flex direction ="column" width="100%" px={5} mt={5}>
                <Button colorScheme="brand" onClick= {()=> onClose()} fontSize={{base: 'sm', lg: 'md'}} variant="outline" size="md" width="100%">Cancel</Button>
                <Button fontSize={{base: 'sm', lg: 'md'}} onClick = {()=> cropImage()} variant="primary" bg="brand.500" color="white" size="md" width="100%" mt={2}>Crop</Button>
            </Flex>
        </Container>
    );
};
