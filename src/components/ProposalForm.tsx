...
export const ProposalForm: FC<ProposalFormType> = ({
    ...
    const validationSchema = yup.object().shape({
        ...
        description: yup
            ...
            .min(100, 'Must be at least 100 digits').max(300, "Cannot be above 300 characters"),
        funder: yup.string().required('This field is required'),
        status: yup.string().required('This field is required'),
        file: mode === "edit" ? yup.string() :  yup.string().required('This field is required'),
    });
                ...
                <Flex direction="column" px={3} pb={5}>
                    <Form>
                            ...
                            >
                                {
                                    STATUSES.map((status, i)=> (
                                        <option value = {status} key={`proposal-status-${i}`}>{status}</option>
                                    ))
                                }
                        ...
                        <FormControl isRequired = {mode==="edit" ? false: true}>
                           <VisuallyHidden>
                              <Input type ="file" name="file-input" id = "file-input" onChange={(e)=> setFieldValue('file', (e.target?.files && e.target.files[0]) || "") }  />
                           </VisuallyHidden>
                           <FormLabel htmlFor='file-input'  borderWidth={1} textAlign="center" py={8} border="1px dashed">
                             {values?.file?.name || (values.fileUrl && values.title) || "Select File"} 
                           </FormLabel>
                           <FormErrorMessage>
