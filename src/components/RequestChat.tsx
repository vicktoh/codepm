...
import { listenOnRequest } from "../services/logsServices";
import {
  markRequestChatAsRead,
  sendRequestChat,
...
export const RequestChat: FC<RequestChatProps> = ({ request }) => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    const unsub = listenOnRequest(request.id, (req) => {
      console.log("I ran");
      setLoading(false);
  ...
  useEffect(() => {
    setTimeout(() => {
      ...
      try {
        markRequestChatAsRead(request.id, conversation, chats.length);
      } catch (error) {
        console.log(error);
      }
    }, 1200);
  ...
  const sendChatMessage = async () => {
    ...
    try {
      setSending(true);
      await sendRequestChat([...(chats || []), newchat], request.id);
    } catch (error) {
      const err: any = error;
      toast({
