...
export const ProjectDetails = () => {
  ...
  return (
    <Flex direction="column">
              ...
              {project.documents?.length ? (
                ...
                >
                  {project.documents.map((document, i) => (
                    <GridItem key={`document-section-${i}`} w="100%">
                      <DocumentCard
                        document={document}
                        onEdit={() => editDocumentPrompt(i)}
                        onDelete={() => deleteDocumentPrompt(i)}
                        key={`document-card-${i}`}
                      />
                    </GridItem>
                  ))}
              ...
              {project?.workplans?.length ? (
                project.workplans.map((workplan, i) => (
                  <WorkplanRow
                    key={`workplan-row-${i}`}
                    workplan={workplan}
                    onEditWorkPlan={() => editWorkplanPrompt(i)}
                    onDeleteWorkPlan={() => deleteWorkplanPrompt(i)}
          ...
          <ModalContent>
            ...
            <ModalBody>
              <Text>{`Are you sure you want to delete this document named ${
                project.documents[selectedDocIndex]?.title || "unknown name"
              }`}</Text>
            </ModalBody>
            <ModalFooter>
      ...
      {project && project.workplans && selectedWorkPlanIndex !== undefined ? (
        <Modal
          isOpen={isDeleteWorkplanOpen}
          onClose={onCloseDeleteWorkplanModal}
          size="sm"
          ...
          <ModalContent>
            ...
            <ModalBody>
              <Text>{`Are you sure you want to delete this workplan named ${
                project.workplans[selectedWorkPlanIndex].title || "unknown name"
